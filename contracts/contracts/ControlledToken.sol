...
/*
    ...
    - Rescue tokens (recover stuck ERC20)
*/
...
contract ControlledToken is ERC20, AccessControl, Pausable, ReentrancyGuard {
    ...
    bytes32 public constant MINTER_ROLE     = keccak256("MINTER_ROLE");     // mint new tokens
    ...
    struct Vesting {
        ...
        uint256 cliff;
        uint256 duration;
        uint256 slice;   // kept for future
        bool revocable;  // kept for future
        bool revoked;    // kept for future
    }
    ...
    event VestingCreated(address user, uint256 amount, uint256 start, uint256 cliff, uint256 duration);
    ...
    constructor(
        ...
        uint256 maxSupply_,     // NEW: cap (0 = unlimited)
        address admin
    ) ERC20(name_, symbol_) {
        ...
        maxBuyAmount = initialSupply;
        maxSellAmount = initialSupply;
    ...
    function mint(address to, uint256 amount) external onlyRole(MINTER_ROLE) {
        ...
        // Optional: if you want limits to auto-expand when supply increases
        // you can update limits manually via setLimits() anyway.
    }
    ...
    function createVesting(
        ...
        // prevent overwriting an active vesting
        require(vesting[user].total == 0 || vesting[user].released == vesting[user].total, "active vesting exists");
        ...
        emit VestingCreated(user, amount, start, cliff, duration);
    }
    ...
    function claimVested() external nonReentrant {
        ...
        // if vesting finished, you can optionally clear storage (gas refund pattern)
        // if (v.released == v.total) { delete vesting[msg.sender]; }
    }
    ...
    // ---------------- RESCUE (stuck tokens) ----------------
    // If someone sends USDT/any ERC20 to this contract by mistake, admin can recover it.
    // Cannot rescue this token itself.
    function rescueERC20(address token, address to, uint256 amount) external onlyRole(DEFAULT_ADMIN_ROLE) {
        require(token != address(this), "no rescue self");
        require(to != address(0), "to=0");
        require(amount > 0, "amount=0");
